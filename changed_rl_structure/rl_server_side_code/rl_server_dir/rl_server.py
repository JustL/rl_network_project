...
class RL_Server(object):
    ...
    __MAX_WAIT_TASKS = 4         # max number of batches that
                                 # a model can handle

    def __init__(self, ip_address, model):

        self._m_server = SimpleXMLRPCServer.SimpleXMLRPCServer(
                (ip_address, RL_Server.__RL_SERVER_PORT_NUM))
    ...
    def unregister_server(self, ip_address):
        self._m_clean_ips.put(ip_address,
                block=True) # use blocking put
