...
def sock_proc(client):
    ...
    tos_val = ctypes.c_uint8(Simple_Flow_Server.REPLY_PRIORITY)

    if libc.setsockopt(client, IPPROTO_IP, IP_TOS,
        ctypes.byref(tos_val), ctypes.sizeof(tos_val)) < 0:

        print "Server cannot set reply priority"
...
class Simple_Flow_Server(object):
    ...
    REPLY_PRIORITY = 0x10        # set all server priorities to
